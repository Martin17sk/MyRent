...
class FinancialService {
  ...
  async getDesempeniosUltimoMes() {
    ...
    return response.data.filter(
      desempenio => new Date(desempenio.fecha).getMonth() === mes,
    )
  }

  async getDesempenioById(id) {
    const response = await axios.get(
      `http://localhost:3000/desempenios?id=${id}`,
    )
  ...
  async getDesempeniosByPerfilId(id) {
    const response = await axios.get(
      `http://localhost:3000/desempenios?perfil_id=${id}}`,
    )
    return response.data
  }
  ...
  async getAllDesempeniosByPropiedadId(id) {
    const response = await axios.get(
      `http://localhost:3000/desempenios?propiedad_id=${id}`,
    )
    return response.data
  }
