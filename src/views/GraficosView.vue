      ...
      <div class="form">
        ...
        <div class="opciones">
          ...
          <DefaultSelect
            ...
            label="Selecciona un gráfico"
          />
          <DefaultSelect
            :options="getNombresPerfiles()"
            v-model="currentPerfilNombre"
            label="Selecciona un perfil"
          />
          <DefaultSelect
            :options="propiedadesOptions"
            :disabled="!isChecked"
            v-model="currentPropiedadNombre"
...
const currentPropiedad = computed(() => {
  ...
  return propiedades.value.find(
    propiedad => propiedad.nombre === currentPropiedadNombre.value,
  )
})
...
const labelMonthMap = {
  ...
  '6 Meses': [
    monthIndex(currentMonth, 6),
    monthIndex(currentMonth, 5),
    monthIndex(currentMonth, 4),
    monthIndex(currentMonth, 3),
    monthIndex(currentMonth, 2),
    monthIndex(currentMonth, 1),
  ].map(month => monthMap[month]),
  '1 Año': [
    monthIndex(currentMonth, 12),
    monthIndex(currentMonth, 11),
    monthIndex(currentMonth, 10),
    monthIndex(currentMonth, 9),
    monthIndex(currentMonth, 8),
    monthIndex(currentMonth, 7),
    monthIndex(currentMonth, 6),
    monthIndex(currentMonth, 5),
    monthIndex(currentMonth, 4),
    monthIndex(currentMonth, 3),
...
const currentPerfilId = computed(() => {
  ...
  return perfiles.value.find(
    perfil => perfil.nombre === currentPerfilNombre.value,
  ).id
})
...
const loadDesempenios = async () => {
  const data =
    isChecked.value && currentPropiedad.value
      ? await service.getAllDesempeniosByPropiedadId(currentPropiedad.value.id)
      : await service.getDesempeniosByPerfilId(currentPerfilId.value)
...
const getPropiedades = async () => {
  propiedades.value = await propiedadesService.getPropiedadesByProfile(
    currentPerfilId.value,
  )
}
...
watch(
  [graphLabel, isChecked, currentPerfilNombre, currentPropiedadNombre],
  () => {
    getPropiedades()
    loadDesempenios()
  },
