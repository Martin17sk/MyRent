    ...
    <MainLayout>
        <form class="form" @submit.prevent="submitForm">
            <div class="left-side">
                ...
                <div class="inputs">
                    <InputLabelVModel v-model="form.name" label="Nombre" placeholder="ej.: Cabaña Luxury"
                        widthProp="100%" heightProp="200px" isObligatory="true" />
                    <InputLabelVModel v-model="form.direccion" label="Dirección" placeholder="ej.: Vicente Salazar 143"
                        widthProp="100%" heightProp="200px" isObligatory="true" />
                </div>
                <div class="numeric-services">
                    <div class="numeric-info">
                        <InputLabelVModel v-model="form.capacidad" label="Capacidad" placeholder="ej.: 4"
                            widthProp="150px" heightProp="200px" isObligatory="true" />
                        <InputLabelVModel v-model="form.dormitorios" label="Dormitorios" placeholder="ej.: 2"
                            isObligatory="true" />
                        <InputLabelVModel v-model="form.banos" label="Baños" placeholder="ej.: 1" isObligatory="true" />
                        <InputLabelVModel v-model="form.precio" label="Precio de renta" placeholder="ej.: 50000"
                            isObligatory="true" />
                    </div>
                    <div class="services-checks">
                        <div class="amenities">
                            <div class="column">
                                <DefaultCheckbox labelText="Internet" v-model="services.internet" />
                                <DefaultCheckbox labelText="TV" v-model="services.tv_cable" />
                                <DefaultCheckbox labelText="Microondas" v-model="services.microondas" />
                            </div>
                            <div class="column">
                                <DefaultCheckbox labelText="Refrigerador" v-model="services.refrigerador" />
                                <DefaultCheckbox labelText="Lavadora" v-model="services.lavadora" />
                                <DefaultCheckbox labelText="Secadora" v-model="services.secadora" />
                            </div>
                        </div>
                        <div class="divider-hor"></div>
                        <div class="services">
                            <div class="column">
                                <DefaultCheckbox labelText="Servicio de comida" v-model="services.servicio_de_comida" />
                                <DefaultCheckbox labelText="Servicio de limpieza"
                                    v-model="services.servicio_de_limpieza" />
                                <DefaultCheckbox labelText="Permitir mascotas" v-model="services.permitir_mascotas" />
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="right-side">
                ...
                <div class="dimensions">
                    <div class="dimensions-top">
                        ...
                        <div class="dimensions-measures">
                            <DefaultSelect label="" :options="measures" v-model="currentMeasure" />
                        </div>
                    </div>
                    <div class="dimensions-inputs">
                        <InputLabelVModel v-model="form.largo" label="Largo" placeholder="ej.: 10" widthProp="50px" />
                        <InputLabelVModel v-model="form.ancho" label="Ancho" placeholder="ej.: 10" widthProp="50px" />
                        <InputLabelVModel v-model="form.alto" label="Alto" placeholder="ej.: 10" widthProp="50px" />
                    </div>
                </div>
                <div class="inventory">
                    <div class="inventory-top">
                        ...
                        <div class="add-inventory-button">
                            <ImageLabelButton imageName="añadir-icono.svg" label="Agregar objeto" widthImageProp="40px"
                                heightImageProp="40px" class="add-button" @click="clickAddInventoryButton" />
                        </div>
                    </div>
                ...
                <div class="employee">
                    <div class="employee-top">
                        ...
                        <div class="add-employee-button">
                            <ImageLabelButton imageName="añadir-icono.svg" label="Agregar empleado"
                                widthImageProp="40px" heightImageProp="40px" class="add-button"
                ...
                <div class="image">
                    ...
                    <div class="add-image-button">
                        <UploadImage widthProp="600px" heightProp="200px" label="Subir imágen">
            ...
            <ConfirmPopup :visible="openInventoryPopup" title="Agregar objeto" @close="openInventoryPopup = false"
                ...
                <InputLabelVModel label="Nombre" :placeholder="'ej.: Tenedor'" v-model="inventoryObject.name" />
                <InputLabelVModel label="Cantidad" :placeholder="'ej.: 3'" v-model="inventoryObject.cantidad" />
            </ConfirmPopup>

            <ConfirmPopup :visible="openEmployeePopup" title="Seleccione un empleado" @close="openEmployeePopup = false"
                @confirm="addEmployee(selectedEmployee)">
                <DefaultSelect label="Empleados" :options="getEmployeesNames" v-model="selectedEmployee" />
            </ConfirmPopup>
        </form>
    </MainLayout>
</template>
...
const getEmployees = async () => {
    employees.value = await empleadoService.getEmpleadosByProfileId(store.profileId);
};
...
const getEmployeesNames = computed(() => {
    let names = employees.value.map((employee) => employee.nombre).filter(employee => !listEmployees.value.includes(employee));
...
const form = props.propiedad ? reactive({
...
const services = props.propiedad ? reactive({
...
const allowToSubmit = computed(() => {
    return form.name && form.direccion && form.capacidad && form.dormitorios && form.banos && form.precio;
});
    ...
    if (allowToSubmit.value) {
        propiedadesService.createPropiedad(createProperty()).then(propiedadCreada => {
            ...
            listInventoryObjects.value.forEach((object) => {
                ...
                const promesaObjeto = objetoService.addObjeto(objectToAdd).then(objetoCreado => {
                    const inventario = {
            ...
            listEmployees.value.forEach((nombre) => {
                ...
                const promesaEmpleado = empleadoService.addEmpleadoPropiedad(empleadoPropiedad);
